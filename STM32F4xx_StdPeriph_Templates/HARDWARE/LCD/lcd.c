...
{ 	
   ...
   //PB11--LCD_BL PB10--LCD_REST 
	...
	//PD0--FSMC_D2
	//PD1--FSMC_D3
	//PD3--FSMC_CLK///NO LINE CONNECTION
	//PD4--FSMC_NOE
	//PD5--FSMC_NWE
	//PD6--FSMC_NWAIT//NO LINE CONNECTION
	//PD7--FSMC_NE1
	//PD8--FSMC_D13
	//PD9--FSMC_D14
	//PD10--FSMC_D5
	//PD11--FSMC_A16//NO LINE CONNECTION
	//PD12--FSMC_A17//NO LINE CONNECTION
	//PD13--FSMC_A18//NO LINE CONNECTION
	//PD14--FSMC_D0
	//PD15--FSMC_D1
	...
	//PE0--FSMC_NBL0//NO LINE CONNECTION
	//PE1--FSMC_NBL1//NO LINE CONNECTION
	//PE2--FSMC_A23
	//PE7--FSMC_D4
	//PE8--FSMC_D5
	//PE9--FSMC_D6
	...
	GPIO_PinAFConfig(GPIOD,GPIO_PinSource7,GPIO_AF_FMC);//PD6
	...
	GPIO_PinAFConfig(GPIOE,GPIO_PinSource2,GPIO_AF_FMC);//PE2
	...
	GPIO_ResetBits(GPIOB,GPIO_Pin_10);//reset the LCD
 	delay_ms(100); // delay 100 ms 
	GPIO_SetBits(GPIOB,GPIO_Pin_10);//release the LCD
	
	GPIO_ResetBits(GPIOB,GPIO_Pin_11);//turn on the blank light
	
	GPIO_SetBits(GPIOB,GPIO_Pin_11);//turn on the blank light
	
 	readWriteTiming.FMC_AddressSetupTime = 0x0F;	 //??????(ADDSET)?1?HCLK 1/36M=27ns
	readWriteTiming.FMC_AddressHoldTime = 0x01;	 //??????(ADDHLD)
	readWriteTiming.FMC_DataSetupTime = 60;		 ////??????(DATAST)?9?HCLK 6*9=54ns	 	 
	readWriteTiming.FMC_BusTurnAroundDuration = 0x00;
	readWriteTiming.FMC_CLKDivision = 0x00;
	readWriteTiming.FMC_DataLatency = 0x00;
	readWriteTiming.FMC_AccessMode = FMC_AccessMode_A;	 //??A 
  ...
  writeTiming.FMC_AccessMode = FSMC_AccessMode_A;	 //模式A 	 
		
		
	FSMC_NORSRAMInitStructure.FMC_Bank = FMC_Bank1_NORSRAM1;//  ??????NE3 ,????BTCR[4],[5]?
	...
	FSMC_NORSRAMInitStructure.FMC_MemoryDataWidth = FMC_NORSRAM_MemoryDataWidth_16b;//????????16bit  FMC_NORSRAM_MemoryDataWidth_16b
	...
	FSMC_NORSRAMInitStructure.FMC_ExtendedMode = FMC_ExtendedMode_Disable; // ????????? FSMC_ExtendedMode_Enable
	...
	FSMC_NORSRAMInitStructure.FMC_WriteTimingStruct = &writeTiming; //??????

	FMC_NORSRAMInit(&FSMC_NORSRAMInitStructure);  //???FSMC??
 
 	FMC_NORSRAMCmd(FMC_Bank1_NORSRAM1, ENABLE);  // ??BANK1??3	
